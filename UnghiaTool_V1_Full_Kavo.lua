-- Unghia Tool V1 - Full Script với UI (Kavo)

-- Load Kavo UI Library
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/xHeptc/Kavo-UI-Library/main/source.lua"))()

local Window = Library.CreateLib("Unghia Tool V1", "DarkTheme")

-- Tab Main
local MainTab = Window:NewTab("Main")
local MainSection = MainTab:NewSection("Auto Farm")
MainSection:NewToggle("Auto Farm", "Tự động farm quái", function(state)
    _G.AutoFarm = state
end)

-- Tab ESP
local ESPTab = Window:NewTab("ESP")
local ESPSection = ESPTab:NewSection("Chức năng ESP")
ESPSection:NewToggle("Island ESP", "Hiện tên đảo", function(state)
    IslandESP = state
end)
ESPSection:NewToggle("Player ESP", "Hiện người chơi", function(state)
    ESPPlayer = state
end)
ESPSection:NewToggle("Chest ESP", "Hiện rương", function(state)
    ChestESP = state
end)
ESPSection:NewToggle("Devil Fruit ESP", "Hiện trái ác quỷ", function(state)
    DevilFruitESP = state
end)

-- Tab Server
local ServerTab = Window:NewTab("Server")
local ServerSection = ServerTab:NewSection("Chức năng Server")
ServerSection:NewButton("Server Hop", "Chuyển server khác", function()
    Hop()
end)


-- ============================================
-- BẮT ĐẦU SCRIPT GỐC (đã fix CheckQuest)
-- ============================================
...
local Window = OrionLib:MakeWindow({Name = "UnghiaTool", HidePremium = false, IntroText = "UnghiaBloxFruit", SaveConfig = true, ConfigFolder = "UnghiaBloxFruit"})
----------Tab-----------------------
